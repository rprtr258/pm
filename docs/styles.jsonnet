...
local themes = {
  ...
  "simple": importstr "themes/simple.yaml",
};
local colors = (
  local theme = "simple";
  local data = std.parseYaml(themes[theme]);
  local wtf = if std.type(data) == "array" then data[0] else data; // TODO: wtf
  {[k]: "#"+wtf[k] for k in std.objectFields(wtf)}
);
...
//   }
  ...
  "body.sticky .sidebar": { "position": "fixed", }, // @media(min-width: 48em) { body.sticky .sidebar { position: fixed } }
  "body .sidebar": {
    "padding": "var(--sidebar-padding)",
  },
}