...
pub fn Deserializer(comptime endian: builtin.Endian, comptime packing: Packing, comptime ReaderType: type) type {
    return struct {
        ...
        fn deserializeInt(self: *Self, comptime T: type) (ReaderType.Error || error{EndOfStream})!T {
            ...
            if (int_size == 1) {
                if (t_bit_count == 8) return @bitCast(buffer[0]);
                const PossiblySignedByte = std.meta.Int(@typeInfo(T).Int.signedness, 8);
                return @truncate(@as(PossiblySignedByte, @bitCast(buffer[0])));
            }
            ...
            for (buffer, 0..) |byte, i| {
                switch (endian) {
                    ...
                    .Little => {
                        result |= @as(U, byte) << @as(Log2U, @intCast(u8_bit_count * i));
                    },
                }
            }
            ...
            switch (child_type_id) {
                ...
                .Union => {
                    ...
                    if (info.tag_type) |TagType| {
                        ...
                        inline for (info.fields) |field_info| {
                            if (@intFromEnum(@field(TagType, field_info.name)) == tag) {
                                const name = field_info.name;
                                // const FieldType = field_info.field_type;
    ...
    return struct {
        ...
        fn serializeInt(self: *Self, value: anytype) Error!void {
            ...
            for (&buffer, 0..) |*byte, i| {
                ...
                const shift = @as(Log2U, @intCast(idx * u8_bit_count));
                const v = u_value >> shift;
                byte.* = if (t_bit_count < u8_bit_count) v else @as(u8, @truncate(v));
            }
        ...
        pub fn serialize(self: *Self, value: anytype) Error!void {
            ...
            switch (@typeInfo(T)) {
                .Void => return,
                .Bool => try self.serializeInt(@as(u1, @intFromBool(value))),
                ...
                .Optional => {
                    if (value == null) {
                        try self.serializeInt(@as(u1, @intFromBool(false)));
                        return;
                    }
                    try self.serializeInt(@as(u1, @intFromBool(true)));
                ...
                .Enum => {
                    try self.serializeInt(@intFromEnum(value));
                },
...
fn testIntSerializerDeserializer(comptime endian: builtin.Endian, comptime packing: Packing) !void {
    ...
    const total_bytes = comptime blk: {
        var bytes = 0;
        comptime var i = 0;
        while (i <= max_test_bitsize) : (i += 1) bytes += (i / 8) + @intFromBool(i % 8 > 0);
        break :blk bytes * 2;
    };
    ...
    const extra_packed_byte = @intFromBool(total_bits % u8_bit_count > 0);
