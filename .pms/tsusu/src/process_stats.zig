...
fn readStatsFile(path: []const u8) !StatsFile {
    var stat_file = try std.fs.cwd().openFile(path, .{.mode=.read_only});
    ...
    var line_it = std.mem.split(u8, stat_line, " ");
...
fn fetchUptime() !u64 {
    var uptime_file = try std.fs.cwd().openFile("/proc/uptime", .{.mode=.read_only});
    defer uptime_file.close();

    var uptime_buffer: [64]u8 = undefined;
    const uptime_str = try uptime_file.reader().readUntilDelimiterOrEof(&uptime_buffer, ' ');
...
fn fetchTotalMemory() !u64 {
    var meminfo_file = try std.fs.cwd().openFile("/proc/meminfo", .{.mode=.read_only});
...
fn readStatmFile(statm_path: []const u8) !StatmFile {
    var statm = try std.fs.cwd().openFile(statm_path, .{.mode=.read_only});
    defer statm.close();
    ...
    var it = std.mem.split(u8, statm_line, " ");
...
pub const ProcessStats = struct {
    ...
    pub fn fetchCPUStats(self: @This(), pid: std.os.pid_t) !f64 {
        ...
        const stat_path = try std.fmt.bufPrint(&path_buffer, "/proc/{any}/stat", .{pid});
        ...
        return @as(f64, 100) *
            ((@as(f64, @floatFromInt(total_time)) / @as(f64, @floatFromInt(self.clock_ticks))) / @as(f64, @floatFromInt(seconds)));
    }

    pub fn fetchMemoryUsage(_: @This(), pid: std.os.pid_t) !u64 {
        var path_buffer: [64]u8 = undefined;
        // calculate ram usage
        const statm_path = try std.fmt.bufPrint(&path_buffer, "/proc/{any}/statm", .{pid});
