...
pub const ServiceLogger = struct {
    ...
    pub fn handleProcessStream(ctx: Context, typ: Std, fd: std.os.fd_t, file: std.fs.File) !void {
        ...
        for (ctx.service.logger_client_fds.items) |client_fd| {
            ...
            serializer.serialize(@as(u8, if (typ == .Out) 2 else 3)) catch |err| {
                std.debug.print("got error on writing to client fd {any}: {any}", .{ client_fd, err });
                continue;
            };

            serializer.serialize(@as(u16, @intCast(msg.len))) catch |err| {
                std.debug.print("got error on writing to client fd {any}: {any}", .{ client_fd, err });
                continue;
            };
    ...
    fn sendError(_: Context, serializer: anytype, error_message: []const u8) void {
        serializer.serialize(@as(u8, 1)) catch return;
        serializer.serialize(@as(u16, error_message.len)) catch return;
    ...
    pub fn handleSignalMessage(ctx: Context) !void {
        ...
        switch (@as(LoggerOpCode, @enumFromInt(opcode))) {
            .Stop => {
                ctx.state.logger.info("service logger for {s} got stop signal", .{ctx.service.name});
    ...
    pub fn handler(ctx: Context) !void {
        var sockets = [_]std.os.pollfd{
            .{ .fd = ctx.stdout,     .events = std.os.POLL.IN, .revents = 0 },
            .{ .fd = ctx.stderr,     .events = std.os.POLL.IN, .revents = 0 },
            .{ .fd = ctx.message_fd, .events = std.os.POLL.IN, .revents = 0 },
        };
        ...
        const log_folder = try std.fmt.allocPrint(allocator, "{s}/logs", .{data_dir});
        ...
        ctx.state.logger.info(
            "Opened stdout/stderr log files for {s}, {s}, {s}",
                ...
                if (pollfd.fd == ctx.stdout) {
                    @This().handleProcessStream(ctx, .Out, pollfd.fd, stdout_logfile) catch |err| switch (err) {
                        ...
                        error.NotOpenForReading => {
                            ctx.state.logger.info("service {s} is likely dead", .{ctx.service.name});
                            return;
                        },
                        else => return err,
                    };
